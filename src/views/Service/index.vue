<template>
	<div class="app">
		<div class="xw-header">
			...
			<div class="xw-header-content" @click="showInfo">
				<div>
					<img src="./image/logo.jpg" class="xw-header-avatar">
					<div class="xw-header-title-wrap">
						<div class="xw-header-title">浣洗客服</div>
					</div>
				</div>
			</div>
			<!-- 小薇客服弹出的具体信息 -->
						...
						<div v-else>
							...
							<div class="xw-chat-customer">
								<div class="xw-customer-avantar-wrap">
									<img src="./image/female.jpg" class="xw-customer-avantar">
								</div>
		...
		<div class="xw-footer-wrap" @click="toShowMaskInfo=false">
			<div class="xw-footer-content">
				...
				<div class="xw-chat-tool">
					...
					<div class="xw-chat-tool-item">
						 <a class="el-icon-picture-outline-round" style="font-size: .24rem; padding-right: .1rem;"  @click="showMoreOpratin=!showMoreOpratin"><van-icon name="photo-o" /> </a>
							...
							<div class="xw-window-text" v-if="showMoreOpratin">
								<label for="files">
									<span v-if='imgs.length < size'>
										<input type="file" name="image" accept="image/*" multiple style="display:none;" id="files" @change='fileChange'>图片
									</span>
								</label>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			let data = {
				status: 'searchUser',
				sendid: localStorage.getItem('userid'), //用户索引
				product: this.$route.query.id, //商品id
			...
			fileChange() { // 加入图片
				...
				for (var i = 0; i < leng; i++) {
					...
					reader.onload = function(e) {
						...
						let data = {
							status: 'send',
							product: _this.$route.query.id, //商品id
							sendid: localStorage.getItem('userid'), //用户索引
							//sendid: 60618,
							message: '',
							img: e.target.result
				...
				/*var content = ''
				for (let i = this.imgs.length; i == this.imgs.length; i++) {
					this.content = this.imgs[this.imgs.length - 1]
					this.records.push({
						time: new Date().toLocaleTimeString(),
						message: '',
						img: this.imgs[this.imgs.length - 1],
						from: 1
					});
					let data = {
						status: 'send',
